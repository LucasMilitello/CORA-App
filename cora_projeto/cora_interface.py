...
APP_ID = "CORA"
APP_ICON_PNG_NAME = "Logo_GIM_02_icon.png"
APP_ICON_ICO_NAME = "Logo_GIM_02.ico"
...
UI_TEXT: dict[str, dict[str, str]] = {
    ...
    "figure.file": {"pt": "Arquivo", "en": "File"},
    ...
    "roi.edit.queue_message": {
        "pt": "Imagens desmarcadas para editar: {count}\n\nO editor permanecera aberto durante toda a fila.\nAo confirmar uma ROI com Enter, a proxima imagem sera carregada na mesma janela. A janela fecha somente apos confirmar a ultima imagem.\n\nDeseja continuar?",
        "en": "Unchecked images to edit: {count}\n\nThe editor will remain open for the entire queue.\nAfter confirming an ROI with Enter, the next image is loaded in the same window. The window closes only after the last image is confirmed.\n\nDo you want to continue?",
    },
...
def resolve_app_resource_path(filename: str) -> Path | None:
    """Resolve recursos tanto no codigo-fonte quanto no bundle do PyInstaller."""
    candidates: list[Path] = []
    frozen_base = getattr(sys, "_MEIPASS", None)
    if frozen_base:
        candidates.append(Path(str(frozen_base)) / filename)
    candidates.extend(
        [
...
class CORAApp:
    ...
    def __init__(
        ...
        self._figure_resize_after_id: str | None = None
    ...
    def _apply_window_icon(self) -> None:
        """Aplica o logo do GIM como icone da janela principal."""
        ico_path = resolve_app_resource_path(APP_ICON_ICO_NAME)
        ...
        png_path = resolve_app_resource_path(APP_ICON_PNG_NAME)
        if png_path is None:
            return
        try:
    ...
    def _on_close(self) -> None:
        ...
        try:
            if self._figure_resize_after_id is not None:
                self.root.after_cancel(self._figure_resize_after_id)
                self._figure_resize_after_id = None
        except Exception:
            pass
    ...
    def _poll_worker_queue(self) -> None:
        ...
        while True:
            ...
            if tag == "item_ok":
                ...
                prefix = self._t("processing.prefix_full") if mode == "full" else self._t("processing.prefix_reprocess")
                elapsed = (
    ...
    def _redefine_roi_for_keys(
        ...
        aborted = False
        total = len(keys)
        editor_session: dict[str, object] = {}
        editable_indexes = [
            idx
            for idx, (group_key, time_tag) in enumerate(keys, start=1)
            if self.processed_by_group.get(group_key, {}).get(time_tag) is not None
        ]
        last_editor_index = editable_indexes[-1] if editable_indexes else 0
        for idx, (group_key, time_tag) in enumerate(keys, start=1):
            ...
            while True:
                new_roi = self._open_roi_editor(
                    ...
                    close_on_accept=(idx == last_editor_index),
                )
                if new_roi is None:
                    if not require_all:
                        break
        ...
        remaining_figure = editor_session.pop("figure", None)
        if remaining_figure is not None:
            try:
                _get_pyplot().close(remaining_figure)
            except Exception:
                pass
    ...
    def _on_figure_canvas_resized(self, event=None) -> None:
        """Refaz a quebra dos metadados quando a area do mosaico muda."""
        if self._shutdown_in_progress:
            return
        ...
        if self._figure_resize_after_id is not None:
            try:
                ...
                pass
        self._figure_resize_after_id = self.root.after(120, self._refresh_figure_after_resize)

    def _refresh_figure_after_resize(self) -> None:
        self._figure_resize_after_id = None
        if self._shutdown_in_progress:
            return
        self._refresh_figure()

    def _figure_metadata_layout(self, ncols: int) -> tuple[float, int]:
        """Calcula largura util do texto e fonte para cada imagem."""
        try:
            canvas_width = int(self.canvas.get_tk_widget().winfo_width())
        except Exception:
            canvas_width = 0
        if canvas_width <= 1:
            canvas_width = int(self.figure.get_figwidth() * self.figure.dpi)

        panel_width = max(110.0, (canvas_width - (20.0 * (ncols + 1))) / max(1, ncols))
        font_size = 10 if panel_width >= 300 else (9 if panel_width >= 210 else 8)
        return max(80.0, panel_width * 0.90), font_size

    def _figure_text_width(self, text: str, font_size: int) -> float:
        """Mede texto em pixels usando o mesmo renderer do mosaico."""
        ...
        try:
            ...
            # PT: Fallback conservador para o primeiro desenho, antes do renderer existir. | EN: Conservative fallback for the first draw before the renderer exists.
            return float(len(str(text)) * font_size * 0.75)

    def _split_figure_token(self, token: str, max_width_px: float, font_size: int) -> list[str]:
        """Divide um nome sem espacos em pedacos que caibam no painel."""
        parts: list[str] = []
        remaining = str(token)
        while remaining:
            ...
            while low <= high:
                middle = (low + high) // 2
                if self._figure_text_width(remaining[:middle], font_size) <= max_width_px:
                    best = middle
                    low = middle + 1
                else:
                    high = middle - 1
    ...
    def _wrap_figure_metadata(self, text: str, max_width_px: float, font_size: int) -> list[str]:
        """Quebra o texto pela largura renderizada, inclusive nomes sem espacos."""
        ...
        for word in words:
            ...
            if self._figure_text_width(candidate, font_size) <= max_width_px:
                current = candidate
                continue
            ...
            if self._figure_text_width(word, font_size) <= max_width_px:
                current = word
                continue

            chunks = self._split_figure_token(word, max_width_px, font_size)
    ...
    def _figure_item_title(
        self,
        group_key: str,
        time_tag: str,
        proc: ProcessedTimepoint | None,
        max_width_px: float,
        font_size: int,
    ) -> str:
        """Monta nome e dados em linhas que cabem na coluna da imagem."""
        src_path = self.group_files.get(group_key, {}).get(time_tag)
        if src_path is None and proc is not None:
            src_path = proc.path
        file_name = src_path.name if src_path is not None else "-"

        lines = self._wrap_figure_metadata(
            f"{self._t('figure.file')}: {file_name}",
            max_width_px,
            font_size,
        )
        data_parts = [time_tag.upper()]
        if proc is not None:
            data_parts.append(f"{self._t('figure.area')}={proc.results.area_auto:.0f} px")
            fechamento_px, fechamento_pct = self._closure_vs_0h(group_key, time_tag)
            if fechamento_pct is not None and time_tag != "0h":
                data_parts.append(f"{self._t('figure.closure')}={fechamento_pct:+.1f}%")
            key = (group_key, time_tag)
            if key in self.pending_reprocess:
                data_parts.append(self._t("figure.pending_roi"))
            if key in self.no_area_items:
                data_parts.append(self._t("figure.no_area"))
        lines.extend(self._wrap_figure_metadata(" | ".join(data_parts), max_width_px, font_size))
        return "\n".join(lines)
    ...
    def _refresh_figure(self, group_key: str | None = None) -> None:
        ...
        metadata_width_px, title_font_size = self._figure_metadata_layout(ncols)
        for idx, time_tag in enumerate(group_times, start=1):
            ...
            if proc is not None:
                ...
                ax.set_title(
                    self._figure_item_title(group_key, time_tag, proc, metadata_width_px, title_font_size),
                ...
                if (not err) and (src_path is not None):
                    try:
                        ...
                        ax.set_title(
                            self._figure_item_title(group_key, time_tag, None, metadata_width_px, title_font_size),
                            fontsize=title_font_size,
                            pad=8,
                        )
                    except Exception:
                    ...
                    wrapped_error = "\n".join(
                        self._wrap_figure_metadata(msg, metadata_width_px, title_font_size)
                    )
                    ax.text(0.5, 0.5, wrapped_error, ha="center", va="center", wrap=True)
                    ax.set_title(
                        self._figure_item_title(group_key, time_tag, None, metadata_width_px, title_font_size),
                        fontsize=title_font_size,
                        pad=8,
        ...
        self.figure.tight_layout(pad=1.1, w_pad=1.0, h_pad=1.0)
        self.canvas.draw_idle()
    ...
    def _open_roi_editor(
        ...
        editor_session: dict[str, object] | None = None,
        close_on_accept: bool = True,
    ) -> np.ndarray | None:
        """Abre editor interativo de ROI e retorna a mascara editada (ou None ao cancelar)."""
        try:
            plt = _get_pyplot()
        except Exception as exc:
        ...
        fig = editor_session.get("figure") if editor_session is not None else None
        figure_is_open = False
        if fig is not None:
            try:
                figure_is_open = bool(plt.fignum_exists(fig.number))
            except Exception:
                figure_is_open = False
        if figure_is_open:
            fig.clear()
        ...
        def _on_editor_close(_event) -> None:
            _remember_window_state()
            if editor_session is not None:
                fig.canvas.stop_event_loop()

        cid_close = fig.canvas.mpl_connect("close_event", _on_editor_close)
        ...
        if editor_session is not None and not plt.fignum_exists(fig.number):
            editor_session.pop("figure", None)
