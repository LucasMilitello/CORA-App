"""Ponto de entrada da interface grafica da aplicacao CORA (Cell Open-Region Analyzer)."""
...
REQUIRED_IMPORTS = ("cv2", "matplotlib", "numpy", "scipy", "sklearn", "tifffile")


def _resolve_venv_python(project_root: Path) -> Path | None:
    """Localiza o Python da .venv local quando ele existir."""
    candidates = (
...
def _bootstrap_project_python() -> None:
    ...
    raise ModuleNotFoundError(
        ...
        f"{missing_text}. Execute com {project_root / '.venv' / 'Scripts' / 'python.exe'} "
