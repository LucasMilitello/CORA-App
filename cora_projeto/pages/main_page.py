...
def build_main_layout(app, time_order: tuple[str, ...]) -> None:
    ...
    ttk.Entry(config_card, textvariable=app.folder_var).grid(row=1, column=1, sticky=tk.EW, padx=6, pady=(10, 0))
    ...
    ttk.Entry(config_card, textvariable=app.output_folder_var).grid(row=2, column=1, sticky=tk.EW, padx=6, pady=(8, 0))
    ...
    app.scan_btn.grid(row=3, column=0, columnspan=4, sticky=tk.EW, pady=(12, 0))
    ...
    canvas_widget.pack(fill=tk.BOTH, expand=True, pady=(6, 0))
    canvas_widget.bind("<Configure>", app._on_figure_canvas_resized, add="+")

    # PT: O status continua sendo atualizado internamente e exibido nos dialogos de progresso, mas nao ocupa uma faixa permanente na tela de visualizacao.
    # EN: Status is still updated internally and shown in progress dialogs, but it does not occupy a permanent bar on the viewer screen.
    app.viewer_bottom = None
